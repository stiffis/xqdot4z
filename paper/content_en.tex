...
Dimensions, configurations, and aggregation criteria will be agreed upon
before the measurement campaign. Programs, versions, seeds, expected
outputs, and logs will be retained. Unfavorable results will also be
reported. The detailed working protocol remains outside the paper until
these decisions are settled.

\balance
\section{Results and Discussion}
The isolated unit matches the reference in 556\,734 comparisons per
simulator. Both simulators detect eight negative controls; Icarus detects
four RTL mutations. Strict lint passes without waivers. Exhaustiveness is
limited to single-term tuples, not all complete operands. Artifacts and hashes
are retained in the repository.

The immediate integration passes 25 programs in both simulators, including
a campaign of 1056 operations. Retirement, writeback, stores, and operands
are compared; dependencies, branches, reset, and the disabled configuration
are tested. Two integration mutations are detected in Icarus.

MUL matches its reference on 86\,704 vectors per simulator and passes
49 programs in both, covering the four MUL/XQDot4Zi configurations.
Eight negative controls per simulator and four Icarus mutations are detected,
two arithmetic and two forwarding faults.
