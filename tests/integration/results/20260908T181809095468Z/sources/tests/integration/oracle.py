...
def execute(binary, enabled, stop_before=None):
    regs, memory, pc = [0]*32, {}, 0
    retired, stores, qdots = [], [], []
    for _ in range(20000):
        if pc == stop_before:
            return dict(retired=retired, stores=stores, qdots=qdots, regs=regs)
        if pc % 2 or pc+2 > len(binary):
            return dict(retired=retired, stores=stores, qdots=qdots, regs=regs, fault=[1,pc])
        ...
        if length == 2:
            ...
            if word == 0x00100073:
                return dict(retired=retired, stores=stores, qdots=qdots, regs=regs, fault=[3,pc])
            if opcode == 0x0b:
                # Field-by-field decode, deliberately not the encoder's mask helper.
                if not enabled or funct3 != 0 or (word >> 25 & 3) != 0:
                    return dict(retired=retired, stores=stores, qdots=qdots, regs=regs, fault=[2,pc])
