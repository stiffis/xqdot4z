...
def execute(binary, enabled, stop_before=None, *, mul_enabled=False, packed_enabled=False):
    regs, memory, pc = [0]*32, {}, 0
    retired, stores, qdots = [], [], []
    muls = []
    packed_dots = []
    def finish(fault=None):
        result = dict(retired=retired, stores=stores, qdots=qdots, muls=muls,
                      packed_dots=packed_dots, regs=regs)
    ...
    for _ in range(20000):
        ...
        if length == 2:
            ...
            if opcode == 0x0b:
                ...
                if not packed_enabled or funct3 != 0 or word >> 28 != 0 or (word >> 25 & 3) != 0:
                    return finish([2,pc])
                h = word >> 27 & 1
                # Decode/extract directly, without the ISA encoder or RTL model.
                rd = dest
                value = sum(((a >> (4*(i+4*h))) & 15)*signed((b >> (8*i)) & 255,8)
                            for i in range(4))
                packed_dots.append([pc,a,b,h])
            elif opcode == 0x37:
                ...
                rd, value = dest, a+b
            elif opcode == 0x33 and funct3 == 0 and word >> 25 == 32:
                rd, value = dest, a-b
