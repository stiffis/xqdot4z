...
module tb_integration;
  ...
  top #(.IMEM_WORDS(8192), .DMEM_WORDS(64), .IMEM_FILE(""), .CHECKS(0),
        .ENABLE_XQDOT4Z(ENABLE), .ENABLE_MUL(ENABLE_MUL)) dut(
  ...
  // Interrupt a QDot or MUL in E, away from all clock edges. Registers do not reset;
  ...
  always @(posedge clk) begin
    ...
    if (!reset && !reset_done && reset_pc>=0 && dut.rvpipe.dp.ValidE &&
        (dut.rvpipe.dp.QDotE || dut.rvpipe.dp.MulE) && dut.rvpipe.dp.PCE==32'(reset_pc)) begin
      reset=1; reset_done=1;
      #1;
      if (dut.rvpipe.dp.QDotE !== 0 || dut.rvpipe.dp.MulE !== 0 ||
