module datapath #(parameter ENABLE_XQDOT4Z = 0, parameter ENABLE_MUL = 0)(input  clk, reset,
  ...
  instruction_policy #(.ENABLE_XQDOT4Z(ENABLE_XQDOT4Z), .ENABLE_MUL(ENABLE_MUL)) policy(
  ...
  generate if (ENABLE_MUL != 0) begin : mul_enabled
    scalar_mul mul(.lhs(RD1E), .rhs(WriteDataE), .result(MulResultE));
  end else begin : mul_disabled
    assign MulResultE = 32'b0;
  end endgenerate
  assign ExecuteResultE = QDotE ? QDotResultE : (MulE ? MulResultE : ALUResultE);
