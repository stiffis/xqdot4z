...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="xqdot4z-m3-") as temporary:
            ...
            for f7 in range(128):
                for f3 in range(8):
                    word=(f7 << 25) | (2 << 20) | (1 << 15) | (f3 << 12) | (3 << 7) | 0x0b
                    valid=f7%4==0 and f3==0
                    try: fields=decode(word)
                    except ValueError: require(not valid,"Decoder rejected legal encoding")
                    else:
                        require(valid and fields==dict(rd=3,rs1=1,rs2=2,zero_point=f7//8,half=f7//4%2),"Decoder accepted reserved fields")
                    decoded+=1
            ...
            mutations={
                "no_weights_forwarding": (".weights_word(SrcAE)",".weights_word(RD1E)"),
                "ignore_half": (".d({QDotD, InstrD[31:28], InstrD[27]})",".d({QDotD, InstrD[31:28], 1'b0})"),
            }
            expected=json.loads((out/"forwarding.expected.json").read_text())
            for name,(before,after) in mutations.items():
                ...
                binary=build/f"mutant_{name}.vvp"
                run(f"mutant_{name}_build",["iverilog","-g2012","-s","tb_integration","-Ptb_integration.ENABLE=1",
                                           "-o",binary,*[mutant if p==datapath else p for p in rtl],bench])
                words=len((out/"forwarding.mem").read_text().splitlines())
                log=run(f"mutant_{name}",["vvp",binary,f"+PROGRAM={out/'forwarding.mem'}",f"+WORDS={words}"])
                require("M3_PASS diagnostic stop" in log,"Mutation must finish simulation normally")
                try: compare(log,expected,"mutant_"+name)
                except RuntimeError as error:
                    require("mismatch" in str(error),"Unexpected mutation failure")
                    report["mutations_detected"][name]=str(error)
                else: raise RuntimeError(f"Undetected integration mutation: {name}")
